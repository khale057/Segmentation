...
class conv_block(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, k = 3, p = 1):
        ...
        self.k = k
        self.p = p
        # kernel_size was 3 originally, padding = 1
        self.conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=self.k, stride=1, padding=self.p, bias=True),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_ch, out_ch, kernel_size=self.k, stride=1, padding=self.p, bias=True),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True))
...
class U_Net(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1):
        ...
        self.Conv2 = conv_block(filters[0], filters[1])#, k = 5, p =2)
        self.Conv3 = conv_block(filters[1], filters[2])#, k = 5, p =2)
        self.Conv4 = conv_block(filters[2], filters[3])#, k = 5, p =2)
        self.Conv5 = conv_block(filters[3], filters[4])

        self.Up5 = up_conv(filters[4], filters[3])
        self.Up_conv5 = conv_block(filters[4], filters[3])

        self.Up4 = up_conv(filters[3], filters[2])#, k = 5, p=2)
        self.Up_conv4 = conv_block(filters[3], filters[2])

        self.Up3 = up_conv(filters[2], filters[1])#, k = 5, p=2)
        self.Up_conv3 = conv_block(filters[2], filters[1])

        self.Up2 = up_conv(filters[1], filters[0])#, k = 5, p=2)
